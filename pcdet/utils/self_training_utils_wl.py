    ...
    with tqdm(dataloader, desc=f'generate_ps_e{epoch}_M2D3D') as t:
        for data in t:
            ...
            if dataset_name == 'nuscenes':
                img, pts, box, theta_wl, calib_file, frame_id, image_shape, pose, bbox_wl, sample_token = data
            else:
...
def get_iou2d(box, bbox_wl, calib, pose, image_shape, cfg, lidar_cs_rec, lidar_pose_rec, camera_cs_rec, camera_pose_rec):
    if lidar_cs_rec is not None: # nuScenes
        corners = get_corners_nus(box) # (3, 8)
        in_front = np.argwhere(corners[1, :] > 0).flatten()
    else:
