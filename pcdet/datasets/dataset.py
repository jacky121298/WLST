...
class DatasetTemplate(torch_data.Dataset):
    ...
    def prepare_data_wl_nuscenes(self, input_dict, nusc):
        ...
        camera_token = s_record['data']['CAM_FRONT']
        ...
        pts_img = np.copy(view_points(pts_camera.points, input_dict['cam_intrinsic'], normalize=True))
        pts_img = pts_img[:2, :].T # (N, 2)

        # For cam_intrinsic
        fu = input_dict['cam_intrinsic'][0, 0]
        fv = input_dict['cam_intrinsic'][1, 1]
        cu = input_dict['cam_intrinsic'][0, 2]
        cv = input_dict['cam_intrinsic'][1, 2]

        for i, box in enumerate(gt_boxes):
            if input_dict['gt_names'][i] not in input_dict['dataset_cfg'].CLASS_NAMES:
                continue

            box2d = Box(box[:3], box[[4, 3, 5]], Quaternion(axis=[0, 0, 1], radians=box[6]))
            box2d = self.lidar_to_camera_box(box2d, lidar_cs_rec, lidar_pose_rec, camera_cs_rec, camera_pose_rec)

            # Filter out the corners that are not in front of the calibrated sensor
            # corners_3d = self.get_corners(box2d)
            corners_3d = box2d.corners()
            in_front = np.argwhere(corners_3d[2, :] > 0).flatten()
            corners_3d = corners_3d[:, in_front]

            # Project 3d box to 2d
            corner_coords = view_points(corners_3d, input_dict['cam_intrinsic'], True).T[:, :2].tolist()

            # Keep only corners that fall within the image
            final_coords = self.post_process_coords(corner_coords, (image.shape[1], image.shape[0]))
            ...
            val_flag_1 = np.logical_and(pts_img[:, 0] >= min_x, pts_img[:, 0] <= max_x)
            val_flag_2 = np.logical_and(pts_img[:, 1] >= min_y, pts_img[:, 1] <= max_y)
            val_flag_merge = np.logical_and(val_flag_1, val_flag_2)
            ...
            if input_dict['dataset_cfg'].WEAK_LABEL.TRANSFER_TO_CENTER:
                ...
                pts = self.lidar_to_camera_pc(pts, lidar_cs_rec, lidar_pose_rec, camera_cs_rec, camera_pose_rec)
                pts.rotate(Quaternion(axis=[0, 1, 0], radians=theta + np.pi / 2).rotation_matrix)
                pts = self.camera_to_lidar_pc(pts, lidar_cs_rec, lidar_pose_rec, camera_cs_rec, camera_pose_rec)
                
                box2d.rotate(Quaternion(axis=[0, 1, 0], radians=theta + np.pi / 2))
                theta_wl.append(theta + np.pi / 2)

            box2d = self.camera_to_lidar_box(box2d, lidar_cs_rec, lidar_pose_rec, camera_cs_rec, camera_pose_rec)
            if input_dict['dataset_cfg'].get('SHIFT_COOR', None):
                pts.points += np.array(input_dict['dataset_cfg'].SHIFT_COOR, dtype=np.float32).reshape((3, 1))
                box2d.center += np.array(input_dict['dataset_cfg'].SHIFT_COOR, dtype=np.float32)

            box2d = np.concatenate((box2d.center.reshape((1, 3)), box2d.wlh.reshape((1, 3)), np.array(box2d.orientation.yaw_pitch_roll[0]).reshape((1, 1))), axis=1)
            box2d = np.squeeze(box2d)[[0, 1, 2, 4, 3, 5, 6]]
            ...
            for b in border:
                ...
                if input_dict['dataset_cfg'].WEAK_LABEL.TRANSFER_TO_CENTER:
                    pt = self.lidar_to_camera_pc(pt, lidar_cs_rec, lidar_pose_rec, camera_cs_rec, camera_pose_rec)
                    pt.rotate(Quaternion(axis=[0, 1, 0], radians=theta + np.pi / 2).rotation_matrix)
                    pt = self.camera_to_lidar_pc(pt, lidar_cs_rec, lidar_pose_rec, camera_cs_rec, camera_pose_rec)
                if input_dict['dataset_cfg'].get('SHIFT_COOR', None):
