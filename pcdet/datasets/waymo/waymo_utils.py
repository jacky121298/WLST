...
def generate_labels_wl(frame):
    obj_name, difficulty, dimensions, locations, heading_angles = [], [], [], [], []
    tracking_difficulty, speeds, accelerations, obj_ids = [], [], [], []
    bbox, bbox_where, num_points_in_gt = [], [], []
    laser_labels = frame.laser_labels
    for i in range(len(laser_labels)):
        box = laser_labels[i].box
        class_ind = laser_labels[i].type
        ...
        for j in range(len(projected_lidar_labels)):
            for k in range(len(projected_lidar_labels[j].labels)):
                if laser_labels[i].id in projected_lidar_labels[j].labels[k].id:
                    box = projected_lidar_labels[j].labels[k].box
                    bbox.append([box.center_x, box.center_y, box.width, box.length])
                    bbox_where.append(CAMERA_NAME[projected_lidar_labels[j].name])
                    has_projected = True
                    break
            if has_projected:
                break
    ...
    annotations['gt_boxes_lidar'] = gt_boxes_lidar
    annotations['bbox'] = np.array(bbox)
    annotations['bbox_where'] = np.array(bbox_where)
    return annotations
...
def process_single_sequence(sequence_file, save_path, sampled_interval, has_label=True, weak_label=False):
    ...
    if weak_label:
        pkl_file = cur_save_dir / ('%s_wl.pkl' % sequence_name)
    else:
    ...
    for cnt, data in enumerate(dataset):
        ...
        image_info = {}
        for j in range(5):
            image = tf.image.decode_jpeg(frame.images[j].image)
            skimage.io.imsave(cur_save_dir / f'{cnt:04d}_{CAMERA_NAME[frame.images[j].name]}.png', image.numpy())
        ...
        for c in frame.context.camera_calibrations:
            info['calib'][CAMERA_NAME[c.name] + '_EX'] = np.array(c.extrinsic.transform, dtype=np.float32).reshape(4, 4)
            info['calib'][CAMERA_NAME[c.name] + '_IN'] = np.array(c.intrinsic, dtype=np.float32)
        
        for l in frame.context.laser_calibrations:
            info['calib'][LASER_NAME[l.name] + '_EX'] = np.array(l.extrinsic.transform, dtype=np.float32).reshape(4, 4)
        
        if has_label:
            if weak_label:
                annotations = generate_labels_wl(frame)
            else:
