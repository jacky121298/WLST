...
class DataAugmentorWL(object):
    def __init__(self, augmentor_configs, class_names, logger=None):
        self.augmentor_configs = augmentor_configs
        self.class_names = class_names
        self.logger = logger
    ...
    def random_object_rotation(self, data_dict=None, config=None):
        ...
        for i in range(len(data_dict['pts_wl'])):
            box, pts = augmentor_utils.rotate_objects(
                data_dict['box_wl'][i].reshape(1, 7),
                data_dict['pts_wl'][i],
                data_dict['gt_boxes_mask'],
                rotation_perturb=config['ROT_UNIFORM_NOISE'],
                prob=config['ROT_PROB'],
            ...
            data_dict['box_wl'][i] = box.squeeze()
            data_dict['pts_wl'][i] = pts
    ...
    def random_object_scaling(self, data_dict=None, config=None):
        ...
        for i in range(len(data_dict['pts_wl'])):
            pts, box = augmentor_utils.scale_pre_object(
                data_dict['box_wl'][i].reshape(1, 7),
                data_dict['pts_wl'][i],
                gt_boxes_mask=data_dict['gt_boxes_mask'],
                scale_perturb=config['SCALE_UNIFORM_NOISE']
    ...
    def random_world_sampling(self, data_dict=None, config=None):
        ...
        for i in range(len(data_dict['pts_wl'])):
            box, pts, gt_boxes_mask = augmentor_utils.global_sampling(
                data_dict['box_wl'][i].reshape(1, 7),
                data_dict['pts_wl'][i],
                gt_boxes_mask=data_dict['gt_boxes_mask'],
                sample_ratio_range=config['WORLD_SAMPLE_RATIO'],
    ...
    def random_world_flip(self, data_dict=None, config=None):
        ...
        for i in range(len(data_dict['pts_wl'])):
            box = data_dict['box_wl'][i].reshape(1, 7)
            pts = data_dict['pts_wl'][i]
            for cur_axis in config['ALONG_AXIS_LIST']:
                assert cur_axis in ['x', 'y']
                box, pts = getattr(augmentor_utils, 'random_flip_along_%s' % cur_axis)(
                    box, pts,
                )
            data_dict['box_wl'][i] = box.squeeze()
            data_dict['pts_wl'][i] = pts
    ...
    def random_world_rotation(self, data_dict=None, config=None):
        ...
        for i in range(len(data_dict['pts_wl'])):
            box, pts = augmentor_utils.global_rotation(
                data_dict['box_wl'][i].reshape(1, 7),
                data_dict['pts_wl'][i],
                rot_range=rot_range,
            )
    ...
    def random_world_scaling(self, data_dict=None, config=None):
        ...
        for i in range(len(data_dict['pts_wl'])):
            box, pts = augmentor_utils.global_scaling(
                data_dict['box_wl'][i].reshape(1, 7),
                data_dict['pts_wl'][i],
    ...
    def normalize_object_size(self, data_dict=None, config=None):
        ...
        for i in range(len(data_dict['pts_wl'])):
            pts, box = augmentor_utils.normalize_object_size(
                data_dict['box_wl'][i].reshape(1, 7),
                data_dict['pts_wl'][i],
                data_dict['gt_boxes_mask'],
                config['SIZE_RES'],
    ...
    def forward(self, data_dict):
        ...
        for i in range(len(data_dict['pts_wl'])):
            data_dict['box_wl'][i][6] = common_utils.limit_period(
                data_dict['box_wl'][i][6], offset=0.5, period=2 * np.pi
            )
