...
def draw_3dbbox(bboxs: np.ndarray, vis: o3d.visualization.Visualizer, color: list, score: np.ndarray=np.array([]), scale: float=1.0, prefix: str=None, disp: bool=False):
    for i, bbox in enumerate(bboxs):
        ...
        if len(score) > 0 and disp:
            vis.add_geometry(text_3d(f'{prefix}: {score[i]:.2f}', [x, y - scale * h, z], direction=(0, 1, 0), degrees=270))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--label1', help='Path to groundtruth labels.')
    parser.add_argument('--label2', help='Path to pseudo labels (M2D3D).')
    parser.add_argument('--label3', default=None, help='Path to pseudo labels (M3D).')
    ...
    annos_path = os.path.join(args.label1, args.seq + '.txt')
    ...
    draw_3dbbox(labels, vis, color=[255, 0, 0])

    # Draw pseudo labels M2D3D (green)
    if '.pkl' in args.label2:
        pseudo, score = [], []
        ps = pickle.load(open(args.label2, 'rb'))
        for key in ps[args.seq]:
            box = ps[args.seq][key]['gt_boxes'] # center
            box[6] += float(key)
            center = calib.lidar_to_rect(box[:3].reshape(1, 3))
            center = roty(-float(key)) @ center.reshape(3, 1)
            box[:3] = np.squeeze(calib.rect_to_lidar(center.reshape(1, 3)))

            pseudo.append(box)
            score.append(ps[args.seq][key]['iou_pred'])
        ...
        pseudo_path = os.path.join(args.label2, args.seq + '.txt')
        ...
        for i in range(len(ps)):
            ...
            if float(p[15]) < args.pos_thres:
                continue
            # (x, y, z, l, w, h, rot)
            pseudo.append(np.array([p[11], p[12], p[13], p[10], p[9], p[8], p[14]]).astype(np.float))
            score.append(float(p[15]))

        pseudo = np.array(pseudo)
        score = np.array(score)
    
    draw_3dbbox(pseudo, vis, color=[0, 255, 0], score=score, scale=1.0, prefix='M2D3D', disp=True)

    # Draw pseudo labels M3D (blue)
    if args.label3 != None:
        ps = pickle.load(open(args.label3, 'rb'))
        pseudo = ps[args.seq]['gt_boxes']
        ...
        pseudo = pseudo[scores >= args.pos_thres]
        pseudo = pseudo[pseudo[:, 7] > 0]
        pseudo = pseudo[:, :7]
        pseudo[:, :3] -= np.array([0.0, 0.0, 1.6])

        pseudo = boxes3d_lidar_to_kitti_camera(pseudo, calib)
        draw_3dbbox(pseudo, vis, color=[0, 0, 255], score=scores, scale=0.0, prefix='M3D', disp=True)
    
    # Run Visualizer
    view_control = vis.get_view_control()
    params = view_control.convert_to_pinhole_camera_parameters()
    params.extrinsic = build_se3_transform([0, 0, 10, 0, 0, 0])
