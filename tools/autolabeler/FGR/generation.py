...
def Find_2d_box(maximum, KeyPoint_3d, box_2d, p2, corner, config, total_pc=None, cluster=None, truncate=False, sample_points=None):
    ...
    if maximum:
        cut_times = max(int(len(KeyPoint) * config.CUT_RATE_MAX), 1)
    else:
        cut_times = min(int(len(KeyPoint) * config.CUT_RATE_MIN), 1)
    ...
    while True:
        ...
        for i in seq:
            try:
                ...
                thresh_min_x = current_min_x + config.RECT_SHRINK_THRESHOLD * (current_max_x - current_min_x)
                thresh_max_x = current_max_x - config.RECT_SHRINK_THRESHOLD * (current_max_x - current_min_x)
                thresh_min_y = current_min_y + config.RECT_SHRINK_THRESHOLD * (current_max_y - current_min_y)
                thresh_max_y = current_max_y - config.RECT_SHRINK_THRESHOLD * (current_max_y - current_min_y)
        ...
        if cut_times == 0 and (Current_FinalPoint[0] - FinalPoint[0]) ** 2 + \
                              (Current_FinalPoint[1] - FinalPoint[1]) ** 2 < config.KEY_VERTEX_MOVE_DIST_THRESH:
            break
        else:
            if cut_times == 0:
                ...
                if maximum:
                    cut_times = max(int(len(KeyPoint) * config.CUT_RATE_MAX_2), 1)
                else:
                    cut_times = min(int(len(KeyPoint) * config.CUT_RATE_MIN), 1)
                ...
                if KeyPoint.shape[0] < config.THRESH_MIN_POINTS_AFTER_DELETING:
                    return None, None, None, None, None, None, None, None
                
                index, KeyPoint, final = delete_noisy_point_cloud(final, Current_Index, KeyPoint, 
                                                                  config.DELETE_TIMES_EVERY_EPOCH)
    ...
    if np.abs(y_max - y_min) < config.MIN_HEIGHT_NORMAL or \
       np.abs(y_max - y_min) > config.MAX_HEIGHT_NORMAL or \
       (truncate == True and (y_max < config.MIN_TOP_TRUNCATE or 
                              y_max > config.MAX_TOP_TRUNCATE or 
                              y_min < config.MIN_BOT_TRUNCATE or 
                              y_min > config.MAX_BOT_TRUNCATE)):
    ...
    if truncate == True or FinalPoint_Weight[0] < config.FINAL_POINT_FLIP_THRESH or \
                           FinalPoint_Weight[1] < config.FINAL_POINT_FLIP_THRESH:
        ...
        if weight[0] <= config.FINAL_POINT_FLIP_THRESH or weight[1] <= config.FINAL_POINT_FLIP_THRESH:
        ...
        # if the angle between bounding box and frustum radiation lines is smaller than config.ANCHOR_FIT_DEGREE_THRESH,
        ...
        loc1, loc2, loc3 = check_anchor_fitting(box, loc1, loc2, loc3, angle_1, angle_2, 
                                                ...
                                                anchor_fit_degree_thresh=config.ANCHOR_FIT_DEGREE_THRESH, 
                                                height_width_rate=config.HEIGHT_WIDTH_RATE, 
                                                height_length_rate=config.HEIGHT_LENGTH_RATE,
                                                length_width_boundary=config.LENGTH_WIDTH_BOUNDARY)
    ...
    if not (config.MIN_WIDTH  <= car_width  <= config.MAX_WIDTH) or \
       not (config.MIN_LENGTH <= car_length <= config.MAX_LENGTH):
...
def gen(base, seg_path, vis_path, label_path, config, save_vis):
    try:
        seq = base

        if not os.path.exists(os.path.join(seg_path, base + '.pickle')):
            assert False, 'no such file: %s' % (os.path.join(seg_path, base + '.pickle'))

        with open(os.path.join(label_path, '%s.txt' % base), 'w') as f:
            pass

        # read pickle file as a dict
        with open(os.path.join(seg_path, base + '.pickle'), 'rb') as f:
            dic = pickle.load(f)
        ...
        for i in objects.keys():
            ...
            img_down, img_side, loc0, loc1, loc2, loc3, y_max, y_min = Find_2d_box(True, pc, objects[i].boxes[0].box,
                                                                                   calib.p2, objects[i].corners, config,
            ...
            if loc2 is not None:
                ...
                if save_vis == True:
                    cv2.imwrite('{:s}/{:.4f}_picture_{:s}_object_{:d}.png'.format(vis_path, std_iou, seq, i), img_down)
                ...
                write_result_to_label(seq, i, CurrentObject, calib, label_path)
...
def write_result_to_label(picture_number, object_number, single_object, calib, label_path):
    ...
    orientation = math.atan2(-1 * (single_object.corners[0][2] - single_object.corners[1][2]),
                           1 * (single_object.corners[0][0] - single_object.corners[1][0]))

    if length < width:
        length, width = width, length
        orientation = math.atan2(-1 * (single_object.corners[2][2] - single_object.corners[1][2]),
    ...
    alpha = orientation + 0.5 * np.pi - math.atan2(center_z, -1 * center_x)
    ...
    with open('%s/%s.txt' % (label_path, picture_number), 'a+') as f:
        f.write(label + '\n')

if __name__ == '__main__':
    # python3 generation.py --dataset_path /home/master/10/jacky1212/dataset/kitti_fgr --seg_path ./kitti/segmentation --gen_path ./kitti/generation --split_path ../../../data/kitti/ImageSets/val.txt --save_vis
    ...
    parser.add_argument('--gen_path', help='Path to save generation results.')
    parser.add_argument('--split_path', help='Path to split file.')
    parser.add_argument('--save_vis', action='store_true', help='Whether to store visualized generation results.')
    ...
    if args.save_vis:
        vis_path = os.path.join(args.gen_path, 'images')
        os.makedirs(vis_path, exist_ok=True)
    
    label_path = os.path.join(args.gen_path, 'labels')
    if os.path.isdir(label_path):
        print(f'Label path {label_path} exists. Removing it.')
        os.system(f'rm -rf {label_path}')
    ...
    print(f'Result will be stored in {args.gen_path}.')

    start = time.time()
    pool = Pool(processes=args.processes)

    for s in split:
        pool.apply_async(gen, (s, args.seg_path, vis_path, label_path, config, args.save_vis))
