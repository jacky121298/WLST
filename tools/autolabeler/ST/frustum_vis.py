...
def get_frustum(points: np.ndarray, color: list):
    lines = [
        [0, 4], [1, 4], [2, 4], [3, 4],
    ]
...
if __name__ == '__main__':
    # python3 frustum_vis.py --pts ./frustum/kitti_pts.npy --gt_box ./frustum/kitti_box.npy --seg ./frustum/kitti_seg.npy --fru ./frustum/kitti_fru.npy
    # python3 frustum_vis.py --pts ./frustum/waymo_pts.npy --gt_box ./frustum/waymo_box.npy --seg ./frustum/waymo_seg.npy --fru ./frustum/waymo_fru.npy
    # python3 frustum_vis.py --pts ./frustum/nus_pts.npy --gt_box ./frustum/nus_box.npy --seg ./frustum/nus_seg.npy --fru ./frustum/nus_fru.npy
    # python3 frustum_vis.py --pts ./frustum/pts.npy --gt_box ./frustum/gt_box.npy --ps_box ./frustum/ps_box.npy --seg ./frustum/seg.npy --fru ./frustum/fru.npy
    parser = argparse.ArgumentParser(description='Frustum visualization.')
    ...
    parser.add_argument('--fru', help='Path to fru.')
    parser.add_argument('--ps_box', default=None, help='Path to ps box.')
    ...
    if pts.shape[1] > 3:
        colors = pts[:, 3:] / 255.0
        pcd.colors = o3d.utility.Vector3dVector(np.ascontiguousarray(colors.astype(np.float64)))
    vis.add_geometry(pcd)

    # Draw frustum (green)
    fru = np.load(args.fru, allow_pickle=True)
    frustum = get_frustum(points=fru, color=[0, 255, 0])
    ...
    # Draw pseudo labels (blue)
    if args.ps_box != None:
        ps_box = np.load(args.ps_box, allow_pickle=True)
        ps_box = ps_box[np.newaxis, ...]
        draw_3dbbox(ps_box, vis, color=[0, 0, 255])
