    ...
    def __getitem__(self, index):
        ...
        if self.fusion_dataset:
            if len(self.sample_token) > 0:
                return self.img_transform(image_wl), torch.from_numpy(_pts), torch.from_numpy(box), theta_wl, calib_file, frame_id, np.array(image_shape), np.array(pose), torch.from_numpy(np.array(bbox_wl)), sample_token
